...
type Cache<T> = {
	...
	putItem(key: string, value: T): void
}

declare function makeCache<T>(): Cache<T>
...
declare function makeBetterCache<T>(validateFunction: (entry: T) => boolean): Cache<T>;
...
//
declare function makeEvenBetterCache<T>(validateFunction: (entry: unknown) => entry is T): Cache<T>;
